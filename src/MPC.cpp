 ...
 public:
  ...
  void operator()(ADvector& fg, const ADvector& vars) {
    ...
    for(int t = 1; t < N; t++) {
      ...
      //computing the value of f(x) at x0
...
vector<double> MPC::Solve(Eigen::VectorXd state, Eigen::VectorXd coeffs) {
  ...
  actuator_values.push_back(solution.x[delta_start]);
  actuator_values.push_back(solution.x[a_start]);
  
  for(int i = 0; i < N - 1; i++) {
    actuator_values.push_back(solution.x[x_start + i + 1]);
    actuator_values.push_back(solution.x[y_start + i + 1]);
  }
